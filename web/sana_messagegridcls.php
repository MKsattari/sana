...
$sana_message_grid = NULL; // Initialize page object first

class csana_message_grid extends csana_message {
	...
	// Page object name
	var $PageObjName = 'sana_message_grid';

	// Grid form hidden field names
	var $FormName = 'fsana_messagegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["sana_message"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->messageID->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_personID") && $objForm->HasValue("o_personID") && $this->personID->CurrentValue <> $this->personID->OldValue)
			return FALSE;
		if ($objForm->HasValue("x__userID") && $objForm->HasValue("o__userID") && $this->_userID->CurrentValue <> $this->_userID->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_messageType") && $objForm->HasValue("o_messageType") && $this->messageType->CurrentValue <> $this->messageType->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_messageText") && $objForm->HasValue("o_messageText") && $this->messageText->CurrentValue <> $this->messageText->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->messageID->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->messageID->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->personID->setOldValue($objForm->GetValue("o_personID"));
		...
		$this->messageType->setOldValue($objForm->GetValue("o_messageType"));
		if (!$this->messageText->FldIsDetailKey) {
			$this->messageText->setFormValue($objForm->GetValue("x_messageText"));
		}
		$this->messageText->setOldValue($objForm->GetValue("o_messageText"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->messageID->CurrentValue = strval($arKeys[0]); // messageID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "sana_person") {
			...
			if ($GLOBALS["sana_person"]->EventCancelled) $this->EventCancelled = TRUE;
		}
