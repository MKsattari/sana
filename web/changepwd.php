...
class cchangepwd extends csana_user {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["sana_user"])) $GLOBALS["sana_user"] = &$this;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'sana_user');
	...
	function Page_Main() {
		...
		if ($bPostBack && $bValidate) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$rsold = $rs->fields;
					if (IsPasswordReset() || ew_ComparePassword($rsold['userPassword'], $this->OldPassword)) {
						$bValidPwd = TRUE;
						if (!IsPasswordReset())
							$bValidPwd = $this->User_ChangePassword($rsold, $sUsername, $this->OldPassword, $this->NewPassword);
						if ($bValidPwd) {
							$rsnew = array('userPassword' => $this->NewPassword); // Change Password
							$rs->Close();
