...
$sana_location_level3 = NULL;

//
// Table class for sana_location_level3
//
class csana_location_level3 extends cTable {
	var $locationLevel3ID;
	var $locationName;
	var $locationLevel1ID;
	var $locationLevel2ID;
	var $locationLevel2Name;
	...
	function __construct() {
		...
		$this->TableVar = 'sana_location_level3';
		$this->TableName = 'sana_location_level3';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`sana_location_level3`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// locationLevel3ID
		$this->locationLevel3ID = new cField('sana_location_level3', 'sana_location_level3', 'x_locationLevel3ID', 'locationLevel3ID', '`locationLevel3ID`', '`locationLevel3ID`', 3, -1, FALSE, '`locationLevel3ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// locationName
		$this->locationName = new cField('sana_location_level3', 'sana_location_level3', 'x_locationName', 'locationName', '`locationName`', '`locationName`', 200, -1, FALSE, '`locationName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['locationName'] = &$this->locationName;

		// locationLevel1ID
		$this->locationLevel1ID = new cField('sana_location_level3', 'sana_location_level3', 'x_locationLevel1ID', 'locationLevel1ID', '`locationLevel1ID`', '`locationLevel1ID`', 3, -1, FALSE, '`locationLevel1ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// locationLevel2ID
		$this->locationLevel2ID = new cField('sana_location_level3', 'sana_location_level3', 'x_locationLevel2ID', 'locationLevel2ID', '`locationLevel2ID`', '`locationLevel2ID`', 3, -1, FALSE, '`locationLevel2ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// locationLevel2Name
		$this->locationLevel2Name = new cField('sana_location_level3', 'sana_location_level3', 'x_locationLevel2Name', 'locationLevel2Name', '`locationLevel2Name`', '`locationLevel2Name`', 200, -1, FALSE, '`locationLevel2Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->fields['locationLevel2Name'] = &$this->locationLevel2Name;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`sana_location_level3`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('locationLevel3ID', $rs))
				ew_AddFilter($where, ew_QuotedName('locationLevel3ID', $this->DBID) . '=' . ew_QuotedValue($rs['locationLevel3ID'], $this->locationLevel3ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`locationLevel3ID` = @locationLevel3ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->locationLevel3ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@locationLevel3ID@", ew_AdjustSql($this->locationLevel3ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "sana_location_level3list.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("sana_location_level3view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("sana_location_level3view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "sana_location_level3add.php?" . $this->UrlParm($parm);
		else
			$url = "sana_location_level3add.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("sana_location_level3edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("sana_location_level3add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("sana_location_level3delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "locationLevel3ID:" . ew_VarToJson($this->locationLevel3ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->locationLevel3ID->CurrentValue)) {
			$sUrl .= "locationLevel3ID=" . urlencode($this->locationLevel3ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["locationLevel3ID"]))
				$arKeys[] = ew_StripSlashes($_POST["locationLevel3ID"]);
			elseif (isset($_GET["locationLevel3ID"]))
				$arKeys[] = ew_StripSlashes($_GET["locationLevel3ID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->locationLevel3ID->CurrentValue = $key;
   ...
   // Common render codes
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->locationLevel2Name->Exportable) $Doc->ExportCaption($this->locationLevel2Name);
				} else {
					if ($this->locationLevel3ID->Exportable) $Doc->ExportCaption($this->locationLevel3ID);
					if ($this->locationName->Exportable) $Doc->ExportCaption($this->locationName);
					if ($this->locationLevel1ID->Exportable) $Doc->ExportCaption($this->locationLevel1ID);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->locationLevel2Name->Exportable) $Doc->ExportField($this->locationLevel2Name);
					} else {
						if ($this->locationLevel3ID->Exportable) $Doc->ExportField($this->locationLevel3ID);
						if ($this->locationName->Exportable) $Doc->ExportField($this->locationName);
						if ($this->locationLevel1ID->Exportable) $Doc->ExportField($this->locationLevel1ID);
	...
	function GetAutoFill($id, $val) {
		$rsarr = array();
		$rowcnt = 0;
		if (preg_match('/^x(\d)*_locationLevel2ID$/', $id)) {
			$conn = &$this->Connection();
			switch (@$gsLanguage) {
				case "en":
					$sSqlWrk = "SELECT `locationName` AS FIELD0 FROM `sana_location_level2`";
					$sWhereWrk = "(`locationLevel2ID` = " . ew_QuotedValue($val, EW_DATATYPE_NUMBER, $this->DBID) . ")";
					break;
				case "fa":
					$sSqlWrk = "SELECT `locationName` AS FIELD0 FROM `sana_location_level2`";
					$sWhereWrk = "(`locationLevel2ID` = " . ew_QuotedValue($val, EW_DATATYPE_NUMBER, $this->DBID) . ")";
			...
			if ($rs = ew_LoadRecordset($sSqlWrk, $conn)) {
				while ($rs && !$rs->EOF) {
					$ar = array();
					$this->locationLevel2Name->setDbValue($rs->fields[0]);
					$this->RowType == EW_ROWTYPE_EDIT;
					$this->RenderEditRow();
					$ar[] = $this->locationLevel2Name->CurrentValue;
					$rowcnt += 1;
					$rsarr[] = $ar;
					$rs->MoveNext();
				}
