...
$sana_location_level4 = NULL;

//
// Table class for sana_location_level4
//
class csana_location_level4 extends cTable {
	var $locationLevel4ID;
	var $locationName;
	var $locationLevel3ID;
	var $locationLevel3Name;
	...
	function __construct() {
		...
		$this->TableVar = 'sana_location_level4';
		$this->TableName = 'sana_location_level4';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`sana_location_level4`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// locationLevel4ID
		$this->locationLevel4ID = new cField('sana_location_level4', 'sana_location_level4', 'x_locationLevel4ID', 'locationLevel4ID', '`locationLevel4ID`', '`locationLevel4ID`', 3, -1, FALSE, '`locationLevel4ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// locationName
		$this->locationName = new cField('sana_location_level4', 'sana_location_level4', 'x_locationName', 'locationName', '`locationName`', '`locationName`', 200, -1, FALSE, '`locationName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['locationName'] = &$this->locationName;

		// locationLevel3ID
		$this->locationLevel3ID = new cField('sana_location_level4', 'sana_location_level4', 'x_locationLevel3ID', 'locationLevel3ID', '`locationLevel3ID`', '`locationLevel3ID`', 3, -1, FALSE, '`locationLevel3ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// locationLevel3Name
		$this->locationLevel3Name = new cField('sana_location_level4', 'sana_location_level4', 'x_locationLevel3Name', 'locationLevel3Name', '`locationLevel3Name`', '`locationLevel3Name`', 200, -1, FALSE, '`locationLevel3Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['locationLevel3Name'] = &$this->locationLevel3Name;

		// locationLevel1ID
		$this->locationLevel1ID = new cField('sana_location_level4', 'sana_location_level4', 'x_locationLevel1ID', 'locationLevel1ID', '`locationLevel1ID`', '`locationLevel1ID`', 3, -1, FALSE, '`locationLevel1ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// locationLevel2ID
		$this->locationLevel2ID = new cField('sana_location_level4', 'sana_location_level4', 'x_locationLevel2ID', 'locationLevel2ID', '`locationLevel2ID`', '`locationLevel2ID`', 3, -1, FALSE, '`locationLevel2ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`sana_location_level4`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('locationLevel4ID', $rs))
				ew_AddFilter($where, ew_QuotedName('locationLevel4ID', $this->DBID) . '=' . ew_QuotedValue($rs['locationLevel4ID'], $this->locationLevel4ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`locationLevel4ID` = @locationLevel4ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->locationLevel4ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@locationLevel4ID@", ew_AdjustSql($this->locationLevel4ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "sana_location_level4list.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("sana_location_level4view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("sana_location_level4view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "sana_location_level4add.php?" . $this->UrlParm($parm);
		else
			$url = "sana_location_level4add.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("sana_location_level4edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("sana_location_level4add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("sana_location_level4delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "locationLevel4ID:" . ew_VarToJson($this->locationLevel4ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->locationLevel4ID->CurrentValue)) {
			$sUrl .= "locationLevel4ID=" . urlencode($this->locationLevel4ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["locationLevel4ID"]))
				$arKeys[] = ew_StripSlashes($_POST["locationLevel4ID"]);
			elseif (isset($_GET["locationLevel4ID"]))
				$arKeys[] = ew_StripSlashes($_GET["locationLevel4ID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->locationLevel4ID->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->locationLevel3Name->EditValue = $this->locationLevel3Name->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->locationLevel3Name->Exportable) $Doc->ExportCaption($this->locationLevel3Name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->locationLevel3Name->Exportable) $Doc->ExportField($this->locationLevel3Name);
