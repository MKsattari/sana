...
$sana_location_level5_add = NULL; // Initialize page object first

class csana_location_level5_add extends csana_location_level5 {
	...
	// Page object name
	var $PageObjName = 'sana_location_level5_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["locationLevel5ID"] != "") {
				...
				$this->setKey("locationLevel5ID", $this->locationLevel5ID->CurrentValue); // Set up key
			} else {
				$this->setKey("locationLevel5ID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "sana_location_level5view.php")
	...
	function LoadDefaultValues() {
		...
		$this->locationLevel4Name->CurrentValue = NULL;
		$this->locationLevel4Name->OldValue = $this->locationLevel4Name->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// locationLevel4Name
		$this->locationLevel4Name->SetDbValueDef($rsnew, $this->locationLevel4Name->CurrentValue, "", FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->locationLevel5ID->setDbValue($conn->Insert_ID());
				$rsnew['locationLevel5ID'] = $this->locationLevel5ID->DbValue;
			}
		} else {
...
// Create page object
if (!isset($sana_location_level5_add)) $sana_location_level5_add = new csana_location_level5_add();

// Page init
$sana_location_level5_add->Page_Init();

// Page main
$sana_location_level5_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$sana_location_level5_add->Page_Render();
?>
...
var CurrentForm = fsana_location_level5add = new ew_Form("fsana_location_level5add", "add");

// Validate form
fsana_location_level5add.Validate = function() {
...
fsana_location_level5add.Form_CustomValidate = 
...
fsana_location_level5add.ValidateRequired = true;
<?php } else { ?>
fsana_location_level5add.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsana_location_level5add.Lists["x_locationLevel4ID"] = {"LinkField":"x_locationLevel4ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_locationName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $sana_location_level5_add->ShowPageHeader(); ?>
<?php
$sana_location_level5_add->ShowMessage();
?>
<form name="fsana_location_level5add" id="fsana_location_level5add" class="<?php echo $sana_location_level5_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($sana_location_level5_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $sana_location_level5_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $sana_location_level5_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fsana_location_level5add.Init();
</script>
<?php
$sana_location_level5_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$sana_location_level5_add->Page_Terminate();
?>
