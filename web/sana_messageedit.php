...
$sana_message_edit = NULL; // Initialize page object first

class csana_message_edit extends csana_message {
	...
	// Page object name
	var $PageObjName = 'sana_message_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->messageID->CurrentValue == "")
			$this->Page_Terminate("sana_messagelist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// personID
			$this->personID->SetDbValueDef($rsnew, $this->personID->CurrentValue, 0, $this->personID->ReadOnly);

			// userID
			$this->_userID->SetDbValueDef($rsnew, $this->_userID->CurrentValue, 0, $this->_userID->ReadOnly);

			// messageType
			$this->messageType->SetDbValueDef($rsnew, $this->messageType->CurrentValue, "", $this->messageType->ReadOnly);

			// messageText
			$this->messageText->SetDbValueDef($rsnew, $this->messageText->CurrentValue, "", $this->messageText->ReadOnly);

			// stationID
			$this->stationID->SetDbValueDef($rsnew, $this->stationID->CurrentValue, NULL, $this->stationID->ReadOnly);

			// messageDateTime
			$this->messageDateTime->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->messageDateTime->CurrentValue, 5), NULL, $this->messageDateTime->ReadOnly);
			...
			$KeyValue = isset($rsnew['personID']) ? $rsnew['personID'] : $rsold['personID'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@personID@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
...
// Create page object
if (!isset($sana_message_edit)) $sana_message_edit = new csana_message_edit();

// Page init
$sana_message_edit->Page_Init();

// Page main
$sana_message_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$sana_message_edit->Page_Render();
?>
...
var CurrentForm = fsana_messageedit = new ew_Form("fsana_messageedit", "edit");

// Validate form
fsana_messageedit.Validate = function() {
...
fsana_messageedit.Form_CustomValidate = 
...
fsana_messageedit.ValidateRequired = true;
<?php } else { ?>
fsana_messageedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsana_messageedit.Lists["x_personID"] = {"LinkField":"x_personID","Ajax":true,"AutoFill":false,"DisplayFields":["x_personName","x_lastName","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fsana_messageedit.Lists["x__userID"] = {"LinkField":"x__userID","Ajax":true,"AutoFill":false,"DisplayFields":["x_personName","x_lastName","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fsana_messageedit.Lists["x_stationID"] = {"LinkField":"x_stationID","Ajax":true,"AutoFill":false,"DisplayFields":["x_stationName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $sana_message_edit->ShowPageHeader(); ?>
<?php
$sana_message_edit->ShowMessage();
?>
<form name="fsana_messageedit" id="fsana_messageedit" class="<?php echo $sana_message_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($sana_message_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $sana_message_edit->Token ?>">
<?php } ?>
...
<?php if ($sana_message->messageID->Visible) { // messageID ?>
	<div id="r_messageID" class="form-group">
		<label id="elh_sana_message_messageID" class="col-sm-2 control-label ewLabel"><?php echo $sana_message->messageID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $sana_message->messageID->CellAttributes() ?>>
...
<input type="hidden" data-table="sana_message" data-field="x_messageID" name="x_messageID" id="x_messageID" value="<?php echo ew_HtmlEncode($sana_message->messageID->CurrentValue) ?>">
<?php echo $sana_message->messageID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
fsana_messageedit.CreateAutoSuggest({"id":"x_personID","forceSelect":false});
</script>
</span>
<?php } ?>
...
if (!$GLOBALS["sana_message"]->UserIDAllow("edit")) $sWhereWrk = $GLOBALS["sana_user"]->AddUserIDFilter($sWhereWrk);
...
<script type="text/javascript">
fsana_messageedit.CreateAutoSuggest({"id":"x__userID","forceSelect":false});
</script>
</span>
...
<script type="text/javascript">
fsana_messageedit.CreateAutoSuggest({"id":"x_stationID","forceSelect":false});
</script>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $sana_message_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fsana_messageedit.Init();
</script>
<?php
$sana_message_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$sana_message_edit->Page_Terminate();
?>
