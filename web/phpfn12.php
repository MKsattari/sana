...
function CurrentProjectID() {
	...
	return "{07091A10-D58A-4784-942B-0E21010F5DFC}";
}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setSessionParentUserID($rs->fields('stationID')); // Load parent User ID
						if (is_null($rs->fields('acl'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('acl'))); // Load User Level
						}
						$this->SetUpUserLevel();
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`stationID` IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('userID'));
