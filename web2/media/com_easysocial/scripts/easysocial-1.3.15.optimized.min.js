...
if(""===f&&b.suggestions().hide(),f==b.lastQueryAddress)return;var g=b.locations[f];g?(b.suggest(g),b.lastQueryAddress=f):b.lookup(f)}},lookup:a._.debounce(function(c){a.GMaps.geocode({address:c,callback:function(a,d){"OK"==d&&(b.locations[c]=a,b.suggest(a),b.lastQueryAddress=c)}})},250),suggest:function(c){var d=b.suggestions();d.hide().empty(),a.each(c,function(a,c){b.view.suggestion({location:c}).data("location",c).appendTo(d)}),d.show()},"{suggestion} activate":function(a){var d=a.data("location");b.navigate(d.latitude,d.longitude)
},"{suggestion} mouseover":function(a){b.suggestion().removeClass("active"),a.addClass("active").trigger("activate")},"{suggestion} click":function(a){var d=a.data("location");b.set(d),b.suggestions().hide()},set:function(a){b.currentLocation=a;var c=a.name;b.textField().val(c),b.latitude().val(a.latitude),b.longitude().val(a.longitude),b.trigger("locationChange",[a])},"{detectLocationButton} click":function(){b.map;a.GMaps.geolocate({success:function(a){EasySocial.ajax("site/controllers/location/getLocations",{latitude:a.coords.latitude,longitude:a.coords.longitude}).done(function(a){b.suggest(a),b.textField().focus()
...
return d}}}}),b.textboxlistLib=b.customTextInput().textboxlist("controller")},getTaggedUsers:function(){var c=[],d=b.customInputItem();return d.length>0&&a.each(d,function(b,d){c.push(a(d).data("id"))}),c},"{customTextInput} addItem":function(a,c,d){var e=b.hiddenCustom().val(),f="";if(""==e)f=d.id;else{var g=e.split(",");g.push(d.id),f=g.join(",")}b.hiddenCustom().val(f)},"{customTextInput} removeItem":function(a,c,d){for(var e=b.hiddenCustom().val(),f="",g=[],h=e.split(","),i=0;i<h.length;i++)h[i]!=d.id&&g.push(h[i]);
f=g.length<=0?"":g.join(","),b.hiddenCustom().val(f)},"{customEditBtn} click":function(){b.customForm().toggle()},"{selection} change":function(a){var c=a.val();"custom"==c?(b.customForm().show(),b.customEditBtn().show()):(b.customForm().hide(),b.customEditBtn().hide())},"{customHideBtn} click":function(){b.customForm().hide(),b.customEditBtn().show(),b.textboxlistLib.autocomplete.hide()}}}),b.resolve()})}),EasySocial.module("site/profile/profile",function(){var b=this;EasySocial.template("info/item",'<li data-profile-apps-item data-layout="custom"><a class="ml-20" href="[%= url %]" title="[%= title %]" data-info-item data-info-index="[%= index %]"><i class="ies-info ies-small mr-5"></i> [%= title %]</a></li>'),EasySocial.require().script("site/profile/header").library("history").done(function(a){EasySocial.Controller("Profile",{defaultOptions:{id:null,"{header}":"[data-profile-header]","{app}":"[data-profile-apps-item]","{action}":"[data-profile-apps-menu]","{contents}":"[data-profile-real-content]","{sidebar}":"[data-sidebar]","{sidebarToggle}":"[data-sidebar-toggle]","{info}":"[data-info]","{infoItem}":"[data-info-item]",view:{infoItem:"info/item"}}},function(b){return{init:function(){b.options.id=b.element.data("id"),b.header().implement(EasySocial.Controller.Profile.Header,{"{parent}":b}),a(function(){b.setLayout()
...
EasySocial.require().view("apps/user/links/story/attachment.item").done(function(){EasySocial.Controller("Story.Links",{defaultOptions:{view:{linkItem:"apps/user/links/story/attachment.item"},urlParser:/(^|\s)(https?:\/\/)?(([a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.([a-z]{2,6}))|(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))(:[0-9]{1,5})?(\/.*)?/gi,"{linkForm}":"[data-story-link-form]","{linkInput}":"[data-story-link-input]","{linkContent}":"[data-story-link-content]","{linkItem}":"[data-story-link-item]","{linkTitle}":"[data-story-link-title]","{linkDescription}":"[data-story-link-description]","{linkImages}":"[data-story-link-images]","{linkImage}":"[data-story-link-image]","{linkVideo}":"[data-story-link-video]","{titleTextfield}":"[data-story-link-title-textfield]","{descriptionTextfield}":"[data-story-link-description-textfield]","{panelButton}":"[data-story-link-panel-button]","{attachButton}":"[data-story-link-attach-button]","{removeButton}":"[data-story-link-remove-button]","{removeThumbnail}":"[data-story-link-remove-image]"}},function(b){return{init:function(){b.linkInput().placeholder()
...
},"{story.textField} input":a._.debounce(function(a){if(!b.currentLink&&!b.crawling){var d=a.val(),e=b.extractUrls(d),f=e[e.length-1];if(f){var f=b.fixUrl(f),g=b.links[f];g&&g.disabled||(b.linkInput().val(f),b.doNotFocus=!0,b.panelButton().click(),b.attachButton().click())}}},750),"{story} save":function(a,c,d){if(b.currentLink){var e={title:b.titleTextfield().val(),description:b.descriptionTextfield().val(),url:b.currentLink.url,video:b.linkVideo().val()};b.removeThumbnail().is(":checked")||(e.image=b.linkImage(".active").attr("src")),d.addData(b,e)
}},"{story} clear":function(){b.linkInput().val(""),b.removeLink()}}}),EasySocial.Controller("Story.Links.Item",{defaultOptions:{"{previousImage}":"[data-story-link-image-prev]","{nextImage}":"[data-story-link-image-next]","{image}":"[data-story-link-image]","{imagesWrapper}":"[data-story-link-images]","{imageIndex}":"[data-story-link-image-index]","{removeThumbnail}":"[data-story-link-remove-image]","{title}":"[data-story-link-title]","{description}":"[data-story-link-description]","{titleTextfield}":"[data-story-link-title-textfield]","{descriptionTextfield}":"[data-story-link-description-textfield]"}},function(b){return{init:function(){},"{removeThumbnail} click":function(){var a=b.removeThumbnail().is(":checked");
