...
$sana_station_edit = NULL; // Initialize page object first

class csana_station_edit extends csana_station {
	...
	// Page object name
	var $PageObjName = 'sana_station_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->stationID->CurrentValue == "")
			$this->Page_Terminate("sana_stationlist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// stationName
			$this->stationName->SetDbValueDef($rsnew, $this->stationName->CurrentValue, "", $this->stationName->ReadOnly);

			// projectID
			$this->projectID->SetDbValueDef($rsnew, $this->projectID->CurrentValue, NULL, $this->projectID->ReadOnly);
			...
			// GPS1
			$this->GPS1->SetDbValueDef($rsnew, $this->GPS1->CurrentValue, "", $this->GPS1->ReadOnly);

			// GPS2
			$this->GPS2->SetDbValueDef($rsnew, $this->GPS2->CurrentValue, "", $this->GPS2->ReadOnly);

			// GPS3
			$this->GPS3->SetDbValueDef($rsnew, $this->GPS3->CurrentValue, "", $this->GPS3->ReadOnly);

			// stationType
			$this->stationType->SetDbValueDef($rsnew, $this->stationType->CurrentValue, "", $this->stationType->ReadOnly);
...
// Create page object
if (!isset($sana_station_edit)) $sana_station_edit = new csana_station_edit();

// Page init
$sana_station_edit->Page_Init();

// Page main
$sana_station_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$sana_station_edit->Page_Render();
?>
...
var CurrentForm = fsana_stationedit = new ew_Form("fsana_stationedit", "edit");

// Validate form
fsana_stationedit.Validate = function() {
...
fsana_stationedit.Form_CustomValidate = 
...
fsana_stationedit.ValidateRequired = true;
<?php } else { ?>
fsana_stationedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsana_stationedit.Lists["x_projectID"] = {"LinkField":"x_projectID","Ajax":true,"AutoFill":false,"DisplayFields":["x_projectName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $sana_station_edit->ShowPageHeader(); ?>
<?php
$sana_station_edit->ShowMessage();
?>
<form name="fsana_stationedit" id="fsana_stationedit" class="<?php echo $sana_station_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($sana_station_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $sana_station_edit->Token ?>">
<?php } ?>
...
<?php if ($sana_station->stationID->Visible) { // stationID ?>
	<div id="r_stationID" class="form-group">
		<label id="elh_sana_station_stationID" class="col-sm-2 control-label ewLabel"><?php echo $sana_station->stationID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $sana_station->stationID->CellAttributes() ?>>
<span id="el_sana_station_stationID">
<span<?php echo $sana_station->stationID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $sana_station->stationID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="sana_station" data-field="x_stationID" name="x_stationID" id="x_stationID" value="<?php echo ew_HtmlEncode($sana_station->stationID->CurrentValue) ?>">
<?php echo $sana_station->stationID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $sana_station_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fsana_stationedit.Init();
</script>
<?php
$sana_station_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$sana_station_edit->Page_Terminate();
?>
