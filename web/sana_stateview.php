...
$sana_state_view = NULL; // Initialize page object first

class csana_state_view extends csana_state {
	...
	// Page object name
	var $PageObjName = 'sana_state_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["stateID"] <> "") {
			$this->RecKey["stateID"] = $_GET["stateID"];
			$KeyUrl .= "&amp;stateID=" . urlencode($this->RecKey["stateID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["stateID"] <> "") {
				...
				$this->RecKey["stateID"] = $this->stateID->QueryStringValue;
			} elseif (@$_POST["stateID"] <> "") {
				$this->stateID->setFormValue($_POST["stateID"]);
				$this->RecKey["stateID"] = $this->stateID->FormValue;
			} else {
				$sReturnUrl = "sana_statelist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "sana_statelist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "sana_statelist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($sana_state_view)) $sana_state_view = new csana_state_view();

// Page init
$sana_state_view->Page_Init();

// Page main
$sana_state_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$sana_state_view->Page_Render();
?>
...
var CurrentForm = fsana_stateview = new ew_Form("fsana_stateview", "view");

// Form_CustomValidate event
fsana_stateview.Form_CustomValidate = 
...
fsana_stateview.ValidateRequired = true;
<?php } else { ?>
fsana_stateview.ValidateRequired = false; 
<?php } ?>
...
<?php $sana_state_view->ExportOptions->Render("body") ?>
<?php
	foreach ($sana_state_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $sana_state_view->ShowPageHeader(); ?>
<?php
$sana_state_view->ShowMessage();
?>
<form name="fsana_stateview" id="fsana_stateview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($sana_state_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $sana_state_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="sana_state">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($sana_state->stateID->Visible) { // stateID ?>
	<tr id="r_stateID">
		<td><span id="elh_sana_state_stateID"><?php echo $sana_state->stateID->FldCaption() ?></span></td>
...
<?php echo $sana_state->stateID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($sana_state->stateClass->Visible) { // stateClass ?>
	<tr id="r_stateClass">
		<td><span id="elh_sana_state_stateClass"><?php echo $sana_state->stateClass->FldCaption() ?></span></td>
...
<?php echo $sana_state->stateClass->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($sana_state->stateName->Visible) { // stateName ?>
	<tr id="r_stateName">
		<td><span id="elh_sana_state_stateName"><?php echo $sana_state->stateName->FldCaption() ?></span></td>
...
<?php if ($sana_state->description->Visible) { // description ?>
	<tr id="r_description">
		<td><span id="elh_sana_state_description"><?php echo $sana_state->description->FldCaption() ?></span></td>
		<td data-name="description"<?php echo $sana_state->description->CellAttributes() ?>>
<span id="el_sana_state_description">
<span<?php echo $sana_state->description->ViewAttributes() ?>>
<?php echo $sana_state->description->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fsana_stateview.Init();
</script>
<?php
$sana_state_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$sana_state_view->Page_Terminate();
?>
