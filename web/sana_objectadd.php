...
$sana_object_add = NULL; // Initialize page object first

class csana_object_add extends csana_object {
	...
	// Page object name
	var $PageObjName = 'sana_object_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["objectID"] != "") {
				...
				$this->setKey("objectID", $this->objectID->CurrentValue); // Set up key
			} else {
				$this->setKey("objectID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "sana_objectview.php")
	...
	function LoadDefaultValues() {
		$this->objectName->CurrentValue = NULL;
		$this->objectName->OldValue = $this->objectName->CurrentValue;
		$this->ownerID->CurrentValue = NULL;
		$this->ownerID->OldValue = $this->ownerID->CurrentValue;
		$this->ownerName->CurrentValue = NULL;
		$this->ownerName->OldValue = $this->ownerName->CurrentValue;
		...
		$this->financialValue->CurrentValue = NULL;
		$this->financialValue->OldValue = $this->financialValue->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// objectName
		$this->objectName->SetDbValueDef($rsnew, $this->objectName->CurrentValue, "", FALSE);

		// ownerID
		$this->ownerID->SetDbValueDef($rsnew, $this->ownerID->CurrentValue, 0, FALSE);

		// ownerName
		$this->ownerName->SetDbValueDef($rsnew, $this->ownerName->CurrentValue, "", FALSE);
		...
		// financialValue
		$this->financialValue->SetDbValueDef($rsnew, $this->financialValue->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->objectID->setDbValue($conn->Insert_ID());
				$rsnew['objectID'] = $this->objectID->DbValue;
			}
		} else {
...
// Create page object
if (!isset($sana_object_add)) $sana_object_add = new csana_object_add();

// Page init
$sana_object_add->Page_Init();

// Page main
$sana_object_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$sana_object_add->Page_Render();
?>
...
var CurrentForm = fsana_objectadd = new ew_Form("fsana_objectadd", "add");

// Validate form
fsana_objectadd.Validate = function() {
...
fsana_objectadd.Form_CustomValidate = 
...
fsana_objectadd.ValidateRequired = true;
<?php } else { ?>
fsana_objectadd.ValidateRequired = false; 
<?php } ?>
...
<?php $sana_object_add->ShowPageHeader(); ?>
<?php
$sana_object_add->ShowMessage();
?>
<form name="fsana_objectadd" id="fsana_objectadd" class="<?php echo $sana_object_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($sana_object_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $sana_object_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $sana_object_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fsana_objectadd.Init();
</script>
<?php
$sana_object_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$sana_object_add->Page_Terminate();
?>
