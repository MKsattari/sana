...
//
// Table class for sana_state
//
class csana_state extends cTable {
	var $stateID;
	var $stateClass;
	var $stateName;
	var $description;
	...
	function __construct() {
		...
		$this->TableVar = 'sana_state';
		$this->TableName = 'sana_state';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`sana_state`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// stateID
		$this->stateID = new cField('sana_state', 'sana_state', 'x_stateID', 'stateID', '`stateID`', '`stateID`', 20, -1, FALSE, '`stateID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->stateID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['stateID'] = &$this->stateID;

		// stateClass
		$this->stateClass = new cField('sana_state', 'sana_state', 'x_stateClass', 'stateClass', '`stateClass`', '`stateClass`', 200, -1, FALSE, '`stateClass`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['stateClass'] = &$this->stateClass;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`sana_state`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('stateID', $rs))
				ew_AddFilter($where, ew_QuotedName('stateID', $this->DBID) . '=' . ew_QuotedValue($rs['stateID'], $this->stateID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->stateID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@stateID@", ew_AdjustSql($this->stateID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("sana_stateview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("sana_stateview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "sana_stateadd.php?" . $this->UrlParm($parm);
		else
			$url = "sana_stateadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("sana_stateedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("sana_stateadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("sana_statedelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "stateID:" . ew_VarToJson($this->stateID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->stateID->CurrentValue)) {
			$sUrl .= "stateID=" . urlencode($this->stateID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["stateID"]))
				$arKeys[] = ew_StripSlashes($_POST["stateID"]);
			elseif (isset($_GET["stateID"]))
				$arKeys[] = ew_StripSlashes($_GET["stateID"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->stateClass->EditValue = $this->stateClass->CurrentValue;
